...
class Pet {
  ...
  load() {
    ...
    this.sprites[this.K_ACTION_RESPOND] = this.loadSprite(
      "texture-pet-responding"
    );
    this.sprites[this.K_ACTION_PROCESS] = this.loadSprite(
      "texture-pet-processing"
    );
  }
  ...
  update() {
    ...
    if (this.action === this.K_ACTION_WALK) {
      window.electronAPI
        .petStep(this.speed * this.orientation, 0)
        .then((response) => {
